...
if [ -z "$(ls -A "$DATADIR" 2>/dev/null)" ]; then
  ...
  mysqld --socket="$SOCKET" --datadir="$DATADIR" --skip-networking &
  until mariadb-admin --socket="$SOCKET" ping >/dev/null 2>&1; do sleep 0.2; done
...
exec mysqld --datadir="$DATADIR" --socket="$SOCKET" --bind-address=0.0.0.0